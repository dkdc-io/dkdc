...
mod tests {
    ...
    fn test_default_config_valid_toml() {
        ...
        assert!(config.get("open").is_some(), "Config should have 'open' section");
        
        let open = config.get("open").unwrap();
        assert!(open.get("aliases").is_some(), "Config should have 'open.aliases' section");
        assert!(open.get("things").is_some(), "Config should have 'open.things' section");
    }
